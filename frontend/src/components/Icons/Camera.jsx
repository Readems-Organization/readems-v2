import React from 'react';

export const Camera = ({ width, height }) => (
  <svg
    width={width}
    height={height}
    viewBox={` 0 0 ${width} ${height}`}
    enableBackground='new 0 0 64 64'
    version='1.1'
    xmlSpace='preserve'
    xmlns='http://www.w3.org/2000/svg'
  >
    <g id='grid' />
    <g id='Layer_2' />
    <g id='Layer_3' />
    <g id='Layer_4' />
    <g id='Layer_5'>
      <g>
        <g>
          <path
            d='M41.65,5.74h-4.938l-1.039-3.25h-6.332     l-1.039,3.25h-3.309V4.268h-3.066v1.555c-1.887,0.249-2.602,1.097-2.602,2.885v10.788c0,2.194,1.064,2.976,4.039,2.976H41.65     c2.977,0,4.041-0.781,4.041-2.976V8.708C45.691,6.521,44.627,5.74,41.65,5.74z M32.143,18.93c-2.668,0-4.846-2.169-4.846-4.846     c0-2.668,2.178-4.837,4.846-4.837c2.676,0,4.846,2.169,4.846,4.837C36.988,16.761,34.818,18.93,32.143,18.93z'
            fill='none'
            stroke='#000000'
            strokeMiterlimit='10'
            strokeWidth='2'
          />
        </g>
      </g>
      <g>
        <path d='M40.158,27.13H24.857v-5.66h15.301V27.13z M26.857,25.13h11.301v-1.66H26.857V25.13z' />
      </g>
      <g>
        <path d='M35.168,62.51h-5.32V25.13h5.32V62.51z M31.848,60.51h1.32V27.13h-1.32V60.51z' />
      </g>
      <g>
        <g>
          <path d='M8.358,62.243l-4.355-3.057l26.266-33.766l1.579,1.126v5.882L8.358,62.243z M6.872,58.756l1.086,0.762l21.89-27.784     v-2.516L6.872,58.756z' />
        </g>
        <g>
          <path d='M55.641,62.254L33.168,32.415v-5.872l1.599-1.133l25.229,33.787L55.641,62.254z M35.168,31.746l20.906,27.76l1.087-0.763     L35.168,29.291V31.746z' />
        </g>
      </g>
    </g>
    <g id='Layer_6' />
    <g id='Layer_7' />
    <g id='Layer_8' />
    <g id='Layer_9' />
    <g id='Layer_10' />
    <g id='Layer_37' />
    <g id='Layer_11' />
    <g id='Layer_12' />
    <g id='Layer_13' />
    <g id='Layer_14' />
    <g id='Layer_16' />
    <g id='Layer_17' />
    <g id='Layer_18' />
    <g id='Layer_19' />
    <g id='Layer_20' />
    <g id='Layer_21' />
    <g id='Layer_22' />
    <g id='Layer_23' />
    <g id='Layer_24' />
    <g id='Layer_25' />
  </svg>
);

Camera.defaultProps = {
  width: '64px',
  height: '64',
};

export default Camera;
